...
c.IS_CHAIN_CLINIC,
s.IS_CONVERTED,
CASE WHEN sum(d.ACTIVE_FINANCE) IS NULL OR C.TRIAL_DURATION = 0 THEN 0 ELSE SUM(d.ACTIVE_FINANCE)/C.TRIAL_DURATION END as ACTIVE_FINANCE_DAYS,
CASE WHEN sum(d.ACTIVE_PATIENT) IS NULL OR C.TRIAL_DURATION = 0 THEN 0 ELSE SUM(d.ACTIVE_PATIENT)/C.TRIAL_DURATION END as ACTIVE_PATIENT_DAYS,
CASE WHEN sum(d.ACTIVE_SETUP) IS NULL OR C.TRIAL_DURATION = 0 THEN 0 ELSE SUM(d.ACTIVE_SETUP)/C.TRIAL_DURATION END as ACTIVE_SETUP_DAYS,
CASE WHEN sum(d.ACTIVE_SCHEDULE) IS NULL OR C.TRIAL_DURATION = 0 THEN 0 ELSE SUM(d.ACTIVE_SCHEDULE)/C.TRIAL_DURATION END as ACTIVE_SCHEDULE_DAYS,
CASE WHEN sum(d.ACTIVE_USER) IS NULL OR C.TRIAL_DURATION = 0 THEN 0 ELSE SUM(d.ACTIVE_USER)/C.TRIAL_DURATION END as ACTIVE_USER_DAYS,
CASE WHEN sum(d.ACTIVE_USER) IS NULL OR C.TRIAL_DURATION = 0 THEN 0 ELSE SUM(d.ACTIVE_USER) END AS ACTIVE_USER_DAYS_RAW
